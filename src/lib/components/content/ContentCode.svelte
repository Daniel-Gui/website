...
<div class="not-prose my-8 overflow-hidden rounded-lg bg-black/5 dark:bg-white/5">
	...
	>
		<div class="flex items-center gap-2">
			{#if data.filename}
				<span class="font-mono">{data.filename}</span>
			{/if}
			{#if data.language}
				<span class="uppercase opacity-50">{data.language}</span>
			{/if}
		</div>
		<button
			onclick={copyCode}
			class="flex items-center gap-1.5 transition-colors hover:text-black dark:hover:text-white"
	...
	<div class="overflow-x-auto p-4">
		<pre class="font-mono text-sm leading-relaxed text-gray-800 dark:text-neutral-200"><code
				>{data.code}</code
			></pre>
	</div>
</div>
